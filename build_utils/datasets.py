...
# 找到图像exif信息中对应旋转信息的key值(orientation)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self,
        ...
        # 查看data文件夹下是否缓存有对应数据集的.shapes文件，里面存储了每张图像的width, height
        ...
        if self.rect:
            ...
            self.img_files = [self.img_files[i] for i in irect]  # Python原生列表不支持用 numpy 数组的批量索引(s[irect])
            ...
            for i in range(nb):
                # bi为分割数据集蒙版，与i依次比较获得bool列表
                ari = ar[bi == i]  # bi: batch index
                # 获取第i个batch中，最小和最大高宽比
                mini, maxi = ari.min(), ari.max()

                # 如果高/宽小于1(w > h)，将w（最大边长）设为img_size
                if maxi < 1:
                    shapes[i] = [maxi, 1]
                # 如果高/宽大于1(w < h)，将h（最大边长）设置为img_size
                elif mini > 1:
                    shapes[i] = [1, 1 / mini]
            # 计算每个batch输入网络的shape值(这里对短边向上取整设置为32的整数倍)得到第i个batch统一相框大小
            # 真实图像放在同一shape中，空余部分用灰色值(114,114,114)填充
        ...
        labels_loaded = False
        # 记录标签的丢失个数、找到个数、空的个数、重复个数
        nm, nf, ne, nd = 0, 0, 0, 0  # number mission, found, empty, duplicate
        # 这里分别命名是为了防止出现rect为False/True时混用导致计算的mAP错误
        # rect=True 会改变图片顺序(根据高宽比排序)；
        # 因此为了保证后面计算mAP时图片、标签、预测结果能正确对应，需要把两种情况下的数据分别保存/命名(rect/norect)，避免混用。
        if rect is True:
            # 得到父文件夹目录./cowboyoutfits_yolo_dataset/train/labels
        ...
        for i, file in enumerate(pbar):
            if labels_loaded is True:
                ...
                try:
                    with open(file, "r") as f:
                        l = np.array([x.split() for x in f.read().splitlines()], dtype=np.float32)  # 得到二维数组
                except Exception as e:
                    print("An error occurred while loading the file {}: {}".format(file, e))
                    nm += 1  # file missing 文件出错
                    continue
            ...
            if l.shape[0]:
                ...
                ne += 1  # file empty 个数为0，l.shape[0]=0(空)
        ...
        # 否则缓存过多浪费显存，还是得保存成numpy格式
        if not labels_loaded and n > 1000:
            print("Saving labels to %s for faster future loading" % np_labels_path)
            np.save(
                np_labels_path,
                np.array(self.labels, dtype=object),
                allow_pickle=True
            )  # save for next time
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        # 这一段就是用插值法，同一输出图片的尺寸(长边缩放至img_size)
...
def load_mosaic(self, index):
    ...
    s = self.img_size  # 416
    # 随机初始化拼接图像的中心点坐标
    xc, yc = [int(random.uniform(s * 0.5, s * 1.5)) for _ in range(2)]  # 随机初始化mosaic center 的x, y坐标
    # 从dataset中随机寻找三张图像进行拼接
    indices = [index] + [random.randint(0, len(self.labels) - 1) for _ in range(3)]  # 当前图片索引和数据集中随机提取的3张图片索引
    # 遍历四张图像进行拼接

...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    lobj = torch.zeros(1, device=device)  # Tensor(0) object损失
    tcls, tbox, indices, anchors = build_targets(p, targets, model)  # 计算所有正样本targets
...
def build_targets(p, targets, model):
    ...
    for i, j in enumerate(model.yolo_layers):  # j: [89, 101, 113]
        ...
        gain[2:] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # gain变成[1,1,grid_w,grid_h,grid_w,grid_h]

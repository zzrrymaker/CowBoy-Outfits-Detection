...
def train(hyp):
    ...
    wdir = "weights" + os.sep  # os.sep：操作系统的路径分隔符Windows是`\`
    best = wdir + "best.pt"
    ...
    accumulate = max(round(64 / batch_size), 1)  # 显卡显存不够，batch_size不能开大。这里累计梯度：多次反向传播攒梯度，攒够N次再更新一次权重，达到大batch的训练效果。
    ...
    # math.fmod(imgsz_test, gs)：取imgsz_test ÷ 32的余数
    ...
    if multi_scale:
        ...
        # 这只是前几个batch的大小，后面循环里面，会重新随机刷新imgsz_train
    ...
    # parse_data_cfg读取 .data数据集配置文件，把文本文件解析成Python字典
    data_dict = parse_data_cfg(data)
    train_path = data_dict["train"]
    val_path = data_dict["valid"]
    nc = 1 if opt.single_cls else int(data_dict["classes"])  # number of classes
    # 相比于coco数据集，类别越少，分类损失权重要同比变小
    hyp["cls"] *= nc / 80
    # 输入图片分辨率越大，obj权重按比例放大。
    hyp["obj"] *= imgsz_test / 320

    # Remove previous results
    # glob.glob()查找匹配通配符的文件/文件夹路径，返回获取的一批文件/文件夹名路径
    ...
    if opt.freeze_layers:
        ...
        # 总共训练3x2=6个parameters 卷积+偏置；yololayer不用训练
    ...
    # optimizer
    pg = [p for p in model.parameters() if p.requires_grad]  # 可训练参数列表
    optimizer = optim.SGD(pg, lr=hyp["lr0"], momentum=hyp["momentum"],
                          weight_decay=hyp["weight_decay"], nesterov=True)
    # 开启混合精度训练amp，即添加梯度缩放器scaler
    # 作用用来实现FP16加速训练、同时避免梯度下溢。（放大loss做backward防止梯度下溢，更新参数梯度时还原）
    scaler = torch.cuda.amp.GradScaler() if opt.amp else None

    start_epoch = 0
    best_map = 0.0  # mAP是所有类别AP(Average Precision)的平均值
    # 筛选尺寸相同的预训练权重并导入到自己模型中。
    if weights.endswith(".pt") or weights.endswith(".pth"):
        # 读取磁盘上pt/pth文件，得到一个字典对象ckpt。
        ckpt = torch.load(weights, map_location=device)

        # load model
        try:
            # state_dict是PyTorch里面nn.Module的内置方法，所有网络模型都可以调用
            # key是字符串，代表这个参数在网络里的完整路径。
        ...
        #     # 把之前训练时那个optimizer的状态，恢复到现在新创建的optimizer里面。
        ...
        if ckpt.get("training_results") is not None:
            with open(results_file, "w") as file:
                file.write(ckpt["training_results"])  # write results.txt与自己的结果进行对比

        # epochs
        # 这里分两种情况：1：如果你的epoch是你想最终训练到的轮数，即epochs > start_epoch。那么后续循环就是range(start_epoch,epochs)
        # 2.如果你的epoch表示你想微调多少训练轮数(额外训练轮数)，即epochs < start_epoch。那么后续循环就是range(start_epoch,start_epoch+epochs)
        # 抽象的讨论
    ...
    # 余弦退火学习率衰减
    # (1 + math.cos(x * math.pi / epochs)) / 2：把范围[-1,1]转换到[0,1]
...
if __name__ == '__main__':
    # 命令行参数解析库
    # 命令行运行代码时，把终端输入的参数读进代码里面，不用改py源代码里的默认参数。
    parser = argparse.ArgumentParser()
    parser.add_argument('--epochs', type=int, default=80)
    parser.add_argument('--batch-size', type=int, default=4)
    parser.add_argument('--cfg', type=str, default='cfg/my_yolov3.cfg', help="*.cfg path")
    parser.add_argument('--data', type=str, default='data/my_data.data', help='*.data path')
    parser.add_argument('--hyp', type=str, default='cfg/hyp.yaml', help='hyperparameters path')  # 超参文件
    ...
    parser.add_argument('--weights', type=str, default='weights/yolov3-spp-ultralytics-608.pt',
